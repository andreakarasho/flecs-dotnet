...
public class PipelineCustomTests
{
    ...
    public void DefaultPipeline_InvalidId_RunsAllSystems()
    {
        var w = new World();
        int hits = 0;
        w.System("a", w.OnUpdate, (W, dt) => hits++);
        w.Progress(0f);
        Assert.Equal(1, hits);
    }
    ...
    public void CustomPipeline_WithTag_OnlyTaggedSystemsRun()
    {
        ...
        var sm = w.System("menu", w.OnUpdate, (W, dt) => menuHits++);
        var sg = w.System("game", w.OnUpdate, (W, dt) => gameHits++);
    ...
    public void SetPipeline_SwitchAtRuntime_ReflectsImmediately()
    {
        var w = new World();
        int menuHits = 0, gameHits = 0;
        var sm = w.System("m", w.OnUpdate, (W, dt) => menuHits++);
        var sg = w.System("g", w.OnUpdate, (W, dt) => gameHits++);
    ...
    {
        var w = new World();
        var s = w.System("x", w.OnUpdate, (W, dt) => { });
