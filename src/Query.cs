...
internal static class QueryUtil
{
    public static bool Matches(Table t, Id[] with, Id[] without, List<Id[]>? orGroups, uint wildcard)
    {
        for (int i = 0; i < with.Length; i++)
            if (!MatchesId(t, with[i], wildcard)) return false;
        ...
        if (orGroups != null)
        {
            for (int g = 0; g < orGroups.Count; g++)
            {
                ...
                for (int i = 0; i < group.Length; i++)
                    if (MatchesId(t, group[i], wildcard)) { any = true; break; }
                if (!any) return false;
            }
        }
        return true;
...
public sealed class Query<T1> : QueryBase where T1 : struct
{
    ...
    public void Each(EachAction<T1> action)
    {
        ...
        for (int ti = 0; ti < _matched.Count; ti++)
        {
            ...
            Column<T1>? col1 = (!_t1Optional || t.Has(_c1))
                ? (Column<T1>?)t.Columns[t.IndexOf(_c1)]
                : null;
            var ents = t.Entities;
            int n = t.Count;
            if (col1 != null)
                for (int r = 0; r < n; r++) action(ents[r], ref col1.GetRef(r));
            else
                for (int r = 0; r < n; r++) action(ents[r], ref Unsafe.NullRef<T1>());
        }
        MarkObserved();
    }
    ...
    public void Each(EachAction<T1, T2> action)
    {
        ...
        for (int ti = 0; ti < _matched.Count; ti++)
        {
            ...
            Column<T1>? col1 = (!_t1Optional || t.Has(_c1))
                ? (Column<T1>?)t.Columns[t.IndexOf(_c1)]
                : null;
            Column<T2>? col2 = (!_t2Optional || t.Has(_c2))
                ? (Column<T2>?)t.Columns[t.IndexOf(_c2)]
                : null;
            var ents = t.Entities;
            int n = t.Count;
            // Fast path — neither optional missing.
            ...
            else
            {
                for (int r = 0; r < n; r++)
                {
                    ref T1 r1 = ref Unsafe.NullRef<T1>();
                    ref T2 r2 = ref Unsafe.NullRef<T2>();
                    if (col1 != null) r1 = ref col1.GetRef(r);
                    if (col2 != null) r2 = ref col2.GetRef(r);
                    action(ents[r], ref r1, ref r2);
                }
            }
        }
        MarkObserved();
...
public sealed class Query<T1, T2, T3> : QueryBase where T1 : struct where T2 : struct where T3 : struct
{
    ...
    public void Each(EachAction<T1, T2, T3> action)
    {
        ...
        for (int ti = 0; ti < _matched.Count; ti++)
        {
            var t = _matched[ti];
            if (t.Count == 0) continue;
            Column<T1>? col1 = (!_t1Optional || t.Has(_c1)) ? (Column<T1>?)t.Columns[t.IndexOf(_c1)] : null;
            Column<T2>? col2 = (!_t2Optional || t.Has(_c2)) ? (Column<T2>?)t.Columns[t.IndexOf(_c2)] : null;
            Column<T3>? col3 = (!_t3Optional || t.Has(_c3)) ? (Column<T3>?)t.Columns[t.IndexOf(_c3)] : null;
            var ents = t.Entities;
            int n = t.Count;
            ...
            else
            {
                for (int r = 0; r < n; r++)
                {
                    ref T1 r1 = ref Unsafe.NullRef<T1>();
                    ref T2 r2 = ref Unsafe.NullRef<T2>();
                    ref T3 r3 = ref Unsafe.NullRef<T3>();
                    if (col1 != null) r1 = ref col1.GetRef(r);
                    if (col2 != null) r2 = ref col2.GetRef(r);
                    if (col3 != null) r3 = ref col3.GetRef(r);
                    action(ents[r], ref r1, ref r2, ref r3);
                }
            }
        }
        MarkObserved();
    ...
    where T1 : struct where T2 : struct where T3 : struct where T4 : struct
{
    ...
    public void Each(EachAction<T1, T2, T3, T4> action)
    {
        ...
        for (int ti = 0; ti < _matched.Count; ti++)
        {
            var t = _matched[ti];
            if (t.Count == 0) continue;
            var col1 = (Column<T1>)t.Columns[t.IndexOf(_c1)]!;
            var col2 = (Column<T2>)t.Columns[t.IndexOf(_c2)]!;
            var col3 = (Column<T3>)t.Columns[t.IndexOf(_c3)]!;
            var col4 = (Column<T4>)t.Columns[t.IndexOf(_c4)]!;
            var ents = t.Entities;
            int n = t.Count;
            for (int r = 0; r < n; r++)
                action(ents[r], ref col1.GetRef(r), ref col2.GetRef(r), ref col3.GetRef(r), ref col4.GetRef(r));
        }
        MarkObserved();
    }
    ...
    where T1 : struct where T2 : struct where T3 : struct where T4 : struct where T5 : struct
{
    ...
    public void Each(EachAction<T1, T2, T3, T4, T5> action)
    {
        ...
        for (int ti = 0; ti < _matched.Count; ti++)
        {
            ...
            var col3 = (Column<T3>)t.Columns[t.IndexOf(_c3)]!;
            var col4 = (Column<T4>)t.Columns[t.IndexOf(_c4)]!;
            var col5 = (Column<T5>)t.Columns[t.IndexOf(_c5)]!;
            var ents = t.Entities;
            int n = t.Count;
            for (int r = 0; r < n; r++)
                action(ents[r], ref col1.GetRef(r), ref col2.GetRef(r), ref col3.GetRef(r), ref col4.GetRef(r), ref col5.GetRef(r));
        }
        MarkObserved();
    }
    ...
    where T1 : struct where T2 : struct where T3 : struct where T4 : struct where T5 : struct where T6 : struct
{
    ...
    public void Each(EachAction<T1, T2, T3, T4, T5, T6> action)
    {
        ...
        for (int ti = 0; ti < _matched.Count; ti++)
        {
            ...
            var col4 = (Column<T4>)t.Columns[t.IndexOf(_c4)]!;
            var col5 = (Column<T5>)t.Columns[t.IndexOf(_c5)]!;
            var col6 = (Column<T6>)t.Columns[t.IndexOf(_c6)]!;
            var ents = t.Entities;
            int n = t.Count;
            for (int r = 0; r < n; r++)
                action(ents[r], ref col1.GetRef(r), ref col2.GetRef(r), ref col3.GetRef(r), ref col4.GetRef(r), ref col5.GetRef(r), ref col6.GetRef(r));
        }
        MarkObserved();
    }
